...
export default function UpdateKategoriPage() {
  ...
  useEffect(() => {
    const fetchCategoryDetails = async () => {
      ...
      try {
        ...
        throw new Error("Tidak dapat memuat rincian kategori.");
      } catch (err) {
        console.warn("Failed fetching from backend, searching local mocks:", err);
        const match = mockCategories.find((cat) => cat.id === id);
        if (match) {
          setNama(match.nama);
        } else {
          setErrorMsg("Data kategori tidak ditemukan.");
        }
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.put(`/kategori/${id}`, { nama });
      const result = response.data;
    ...
    <div className="max-w-xl mx-auto flex flex-col gap-6 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-50">Perbarui Kategori</h2>
        </div>
        <Link
          href="/kategori"
