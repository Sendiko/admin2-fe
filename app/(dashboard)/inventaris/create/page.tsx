...
export default function CreateInventarisPage() {
  ...
  const [idKategori, setIdKategori] = useState("kat-01");
  const [idLokasi, setIdLokasi] = useState("lok-01");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(result.message || "Gagal membuat aset barang.");
      }
      ...
      if (err.message.includes("Failed to fetch")) {
        setSuccessMsg("API Offline: Barang inventaris disimulasikan didaftarkan secara lokal!");
        setTimeout(() => {
          router.push("/inventaris");
        }, 1500);
      } else {
    ...
    <div className="max-w-2xl mx-auto flex flex-col gap-6 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-50">Daftar Aset Inventaris Baru</h2>
        </div>
        <Link
          href="/inventaris"
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all placeholder-slate-400 font-mono"
            />
          </div>
          ...
          >
            {isLoading ? "Menambahkan..." : "Tambah ke Inventaris"}
          </button>
        </form>
      </div>
    </div>
