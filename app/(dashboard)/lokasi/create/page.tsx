...
export default function CreateLokasiPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post("/lokasi", { nama });
      const result = response.data;

      if (!result.success) {
        throw new Error(result.message || "Gagal mendaftarkan lokasi penyimpanan.");
      }

      setSuccessMsg("Lokasi penyimpanan berhasil didaftarkan!");
      setTimeout(() => {
        router.push("/lokasi");
      }, 1500);
      ...
      if (err.message.includes("Failed to fetch")) {
        setSuccessMsg("API Offline: Lokasi penyimpanan disimulasikan secara lokal!");
        setTimeout(() => {
          router.push("/lokasi");
        }, 1500);
      } else {
    ...
    <div className="max-w-xl mx-auto flex flex-col gap-6 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-50">Daftar Lokasi Penyimpanan</h2>
        </div>
        <Link
          href="/lokasi"
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl shadow-sm p-6">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          >
            {isLoading ? "Menyimpan..." : "Tambah Lokasi Penyimpanan"}
          </button>
        </form>
      </div>
    </div>
