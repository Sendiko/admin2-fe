...
export default function LaboratoriumPage() {
  const [labs, setLabs] = useState<Laboratory[]>(mockLabs);
  ...
  useEffect(() => {
    const fetchLabs = async () => {
      try {
        ...
        console.warn("Failed fetching labs, seeking local mock fallback:", err);
      }
    };
    fetchLabs();
  }, []);
  ...
  const filteredLabs = useMemo(() => {
    return labs.filter((lab) => {
      ...
      return lab.nama.toLowerCase().includes(term) || lab.kode.toLowerCase().includes(term);
    });
  }, [labs, searchQuery]);
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setLabs(labs.filter((lab) => lab.id !== id));
      setSuccessMsg("Laboratorium dihapus secara lokal (API offline).");
    }
  };
    ...
    <div className="flex flex-col gap-6 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-50">Ruang Laboratorium (Laboratorium)</h2>
        </div>

        <Link
          href="/laboratorium/create"
      ...
      <div className="relative max-w-sm">
        ...
        <input
          type="text"
          placeholder="Filter ruangan berdasarkan nama atau kode..."
          value={searchQuery}
      ...
      {/* Table list */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden max-w-3xl">
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-800 text-slate-400 font-semibold text-xs bg-slate-50/50 dark:bg-slate-900/50">
                ...
                <th className="py-3 px-4">UUID Ruangan</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80 text-slate-700 dark:text-slate-300">
              {filteredLabs.length > 0 ? (
                filteredLabs.map((lab) => (
                  <tr key={lab.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/10 transition-colors duration-150">
                    ...
                    <td className="py-3.5 px-4 text-center">
                      <div className="flex items-center justify-center gap-3">
                        <Link
                          href={`/laboratorium/update/${lab.id}`}
                ...
                <tr>
                  <td colSpan={4} className="py-8 text-center text-slate-400 font-medium">
                    Tidak ada ruang laboratorium yang terdaftar.
                  </td>
                </tr>
              )}
            </tbody>
