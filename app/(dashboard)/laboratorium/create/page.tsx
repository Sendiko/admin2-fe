...
export default function CreateLaboratoriumPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post("/laboratorium", { kode, nama });
      const result = response.data;

      if (!result.success) {
        throw new Error(result.message || "Gagal mendaftarkan laboratorium.");
      }

      setSuccessMsg("Laboratorium berhasil didaftarkan!");
      setTimeout(() => {
      ...
      if (err.message.includes("Failed to fetch")) {
        setSuccessMsg("API Offline: Ruang laboratorium disimulasikan secara lokal!");
        setTimeout(() => {
          router.push("/laboratorium");
        }, 1500);
      } else {
    ...
    <div className="max-w-xl mx-auto flex flex-col gap-6 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-50">Daftar Laboratorium Baru</h2>
        </div>
        <Link
          href="/laboratorium"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all placeholder-slate-400 font-bold"
            />
          </div>
          ...
          >
            {isLoading ? "Menyimpan..." : "Daftar Laboratorium"}
          </button>
        </form>
      </div>
    </div>
