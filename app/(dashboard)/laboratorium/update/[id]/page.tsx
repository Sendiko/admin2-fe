...
export default function UpdateLaboratoriumPage() {
  ...
  useEffect(() => {
    const fetchLabDetails = async () => {
      ...
      try {
        ...
        throw new Error("Tidak dapat memuat detail laboratorium.");
      } catch (err) {
        console.warn("Failed fetching from backend, searching local mocks:", err);
        const match = mockLabs.find((lab) => lab.id === id);
        if (match) {
          setKode(match.kode);
          setNama(match.nama);
        } else {
          setErrorMsg("Data laboratorium tidak ditemukan.");
        }
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.put(`/laboratorium/${id}`, { kode, nama });
      const result = response.data;

      if (!result.success) {
        throw new Error(result.message || "Gagal memperbarui data laboratorium.");
      }

      setSuccessMsg("Laboratorium berhasil diperbarui!");
      setTimeout(() => {
    ...
    <div className="max-w-xl mx-auto flex flex-col gap-6 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-50">Perbarui Laboratorium</h2>
        </div>
        <Link
          href="/laboratorium"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all placeholder-slate-400 font-bold"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              required
              disabled={isLoading}
              placeholder="Contoh: Laboratorium E1 (Pemrograman)"
              value={nama}
